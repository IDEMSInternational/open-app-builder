...
/**
 * One-time cleanup of asset files downloaded before packs were namespaced on disk.
 *
 * Older builds saved pack files straight into the deployment's storage folder (`{deployment}/
 * images/asset.png`). They now live under `{deployment}/remote_assets/{packName}/`, so the old
 * files are invisible to the resume gate and untouched by `asset_pack: delete` / `reset`, which
 * only clear `remote_assets` - without this they would occupy device storage forever.
 *
...
const migration: Migration<IRemoteAssetMigrationContext> = {
  id: "2026-08-05-namespace-remote-assets",
  ...
  run: async ({ fileManagerService, dynamicDataService }) => {
    try {
      ...
      if (!legacyFilePaths.length) {
        console.log("[REMOTE ASSETS] No pre-namespacing asset files to clean up");
        return;
      }
      console.log(
        `[REMOTE ASSETS] Removing ${legacyFilePaths.length} pre-namespacing asset file(s) from local storage`
      );
      ...
      // Clear pack status so affected packs download again into the new layout. Every row has to go:
      // the old layout stored no pack name in the path, so a legacy file cannot be attributed to the
      // pack it came from. Packs whose files were already namespaced are re-fetched needlessly, but
      // that is a one-off cost and far better than leaving a pack `completed` with no files on disk.
      await dynamicDataService.resetFlow("data_list", ASSET_PACKS_DATA_LIST);
    } catch (error) {
      console.error("[REMOTE ASSETS] Pre-namespacing asset cleanup failed", error);
    }
  },
};
 ...
 * saved under the current per-pack layout. Bundled paths and web CDN URLs never match.
 */

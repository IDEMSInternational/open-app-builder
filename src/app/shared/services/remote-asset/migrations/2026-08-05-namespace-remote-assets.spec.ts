...
import migration, { IRemoteAssetMigrationContext } from "./2026-08-05-namespace-remote-assets";
...
/** A recorded filePath for a file saved under the current per-pack layout */
const namespacedSrc = (relativePath: string, assetPackName = "asset_pack_1") =>
  legacySrc(`remote_assets/${assetPackName}/${relativePath}`);

/**
 * Call standalone tests via:
 * yarn ng test --include src/app/shared/services/remote-asset/migrations/2026-08-05-namespace-remote-assets.spec.ts
 */
describe("Migration: namespace-remote-assets", () => {
  ...
  it("only deletes files it has a record of downloading itself", async () => {
    ...
    // saving as an asset - and anything already namespaced - must be left alone
    dynamicDataService.snapshot.and.resolveTo([
      { id: "images/bundled.png", filePath: "assets/images/bundled.png" },
      { id: "images/namespaced.png", filePath: namespacedSrc("images/namespaced.png") },
  ...
  it("clears pack status so affected packs download again into the new layout", async () => {
    dynamicDataService.snapshot.and.resolveTo([
      { id: "images/asset.png", filePath: legacySrc("images/asset.png") },
    ] as any);

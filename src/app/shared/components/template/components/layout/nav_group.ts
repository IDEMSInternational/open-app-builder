...
@Component({
  ...
  template: `<div class="nav-group">
    <div
      *ngFor="let childRow of _row?.rows; let i = index"
      class="slide"
      [hidden]="i != sectionIndex"
    >
    ...
    <div class="nav-buttons" *ngIf="!navButtonList[sectionIndex].hidden">
      <ion-button
        *ngIf="sectionIndex > 0 && navButtonList[sectionIndex].showBack"
        (click)="backClicked()"
        >{{ navButtonList[sectionIndex].back }}</ion-button
      >
      <ion-button *ngIf="sectionIndex + 1 < _row?.rows?.length" (click)="nextClicked()">{{
        navButtonList[sectionIndex].next
      }}</ion-button>
      <ion-button
        *ngIf="navButtonList[sectionIndex].showSkip && sectionIndex + 1 < _row?.rows?.length"
        (click)="skipClicked()"
      >
        {{ navButtonList[sectionIndex].skip }}
      </ion-button>
      <ion-button
        *ngIf="navButtonList[sectionIndex].finish && sectionIndex + 1 == _row?.rows?.length"
        (click)="finishClicked()"
      >
        {{ navButtonList[sectionIndex].finish }}
      </ion-button>
      <ion-button
        *ngIf="navButtonList[sectionIndex].showRestart && sectionIndex + 1 == _row?.rows?.length"
  ...
  @Input() set row(value: FlowTypes.TemplateRow) {
    ...
    if (value && value.rows) {
      this.navButtonList = value.rows.map((row) => {
        ...
        let restart = paramMap.restart_button_text
          ? paramMap.restart_button_text
          : groupRestartText;

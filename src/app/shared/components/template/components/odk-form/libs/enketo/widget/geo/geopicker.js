...
class Geopicker extends Widget {
    ...
    this.$widget
      ...
      .on('change change.bymap change.bysearch', (event) => {
        ...
        if (
          ...
          that.updatedPolylineWouldIntersect(latLng, that.currentIndex)
        ) {
      ...
      getCurrentPosition(options)
        .then((result) => {
          if (
            that.polyline &&
            that.props.type === 'geoshape' &&
            that.updatedPolylineWouldIntersect(result, that.currentIndex)
          ) {
            that._showIntersectError();
          } else {
    ...
    return this._getLayers().then((layers) => {
      ...
      that.map = L.map(`map${that.mapId}`, options).on('click', (e) => {
        ...
        if (
          ...
          that.updatedPolylineWouldIntersect(latLng, indexToPlacePoint)
        ) {
          that._showIntersectError();
        } else if (
          !that.$lat.val() ||
          ...
          L.marker(that._cleanLatLng(latLng), {
            ...
            .on('dragend', (e) => {
              ...
              if (
                that.polyline &&
                that.props.type === 'geoshape' &&
                that.updatedPolylineWouldIntersect(latLng, index)
              ) {
                that._showIntersectError();
                that._updateMarkers();
              } else {
  ...
  _closePolygon() {
    ...
    if (
      ...
      (this.points.length === 3 && !this._isValidLatLng(this.points[2])) ||
    ...
    if (
      ...
      this.updatedPolylineWouldIntersect(this.points[0], this.currentIndex)
    ) {
  ...
  updatedPolylineWouldIntersect(latLng, index) {
    ...
    if (
      polylinePoints[0][0] === polylinePoints[polylinePoints.length - 1][0] &&
  ...
  get props() {
    ...
    props.wide =
      this.question.clientWidth / this.element.closest('form.or').clientWidth >
      0.8;

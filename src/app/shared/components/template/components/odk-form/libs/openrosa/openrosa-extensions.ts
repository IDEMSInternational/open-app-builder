...
import { DATE_STRING, dateStringToDays, dateToDays, isValidDate } from "./utils/date";
...
const openrosa_xpath_extensions = function () {
  ...
  const func = {
    ...
    position: function (r) {
      ...
      if (arguments.length > 1) throw new Error("too many args");
      if (r && r.t !== "arr")
        throw new Error("wrong arg type for position() - expected nodeset, but got: " + r.t);
    ...
    "selected-at": function (list, index) {
      if (!index)
        throw new Error(
          "No index provided for selected-at() [index=" + index + "; list=" + JSON.stringify(list)
        );
    ...
    case "str":
      ...
      if (RAW_NUMBER.test(r)) {
        ...
        if (isValidDate(temp[0], temp[1], temp[2])) {
          ...
          const time =
            `${_zeroPad(temp[0])}-${_zeroPad(temp[1])}-${_zeroPad(temp[2])}` + timeComponent;

      ...
      switch (rt) {
        ...
        case XPathResult.ANY_TYPE:
          ...
          switch (r.t) {
            ...
            case "arr":
              return toExternalResult(r, XPathResult.UNORDERED_NODE_ITERATOR_TYPE);
            default:
    ...
    const stack = [{ t: "root", tokens: [] }],
      ...
      callFn = function (name, supplied) {
        ...
        for (let i = 0; i < supplied.length; ++i) {
          if (i % 2) {
            if (supplied[i] !== ",")
              throw new Error(
                "Weird args (should be separated by commas):" + JSON.stringify(supplied)
              );
      ...
      callNative = function (name, args) {
        ...
        for (let i = 0; i < args.length; ++i) {
          ...
          switch (arg.t) {
            case "arr":
              throw new Error(`callNative() can't handle nodeset functions yet for ${name}()`);
            case "bool":
              argString += arg.v + "()";
              break;
            case "num":
            ...
            case "str": {
              ...
              if (arg.v.indexOf(quote) !== -1)
                throw new Error(
                  "Quote character found in String Literal: " + JSON.stringify(arg.v)
                );
      ...
      handleXpathExpr = function () {
        ...
        if (prev && prev.t === "arr") {
          ...
          const contextNode =
            cN?.nodeType === Node.ATTRIBUTE_NODE && expr.startsWith("/") ? cN.ownerDocument : cN;

          pushToken(
            toInternalResult(wrapped.evaluate(expr, contextNode, nR, XPathResult.ANY_TYPE, null))
          );
        }
          ...
          if (--cur.depth) {
            ...
            const filteredNodes = contextNodes.filter((cN, i) => {
              const res = toInternalResult(
                evaluate(expr, cN, nR, XPathResult.ANY_TYPE, null, contextNodes.length, i + 1)
              );

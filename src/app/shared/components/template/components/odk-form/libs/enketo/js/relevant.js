...
export default {
  ...
  updateNodes(nodes, forceClearNonRelevant = false, options = {}) {
    ...
    nodes.forEach((node) => {
      ...
      const insideRepeatClone =
        ...
        (clonedRepeatsPresent && branchNode.closest('.or-repeat.clone'));
      ...
      p.ind =
        ...
        (context && insideRepeatClone ? this.form.input.getIndex(node) : 0);
      ...
      if (
        this.process(branchNode, p.path, result, forceClearNonRelevant, {
          ...options,
          repeatIndex: p.ind,
          repeatPath,
        }) === true
  ...
  toggleNonRelevantModelNodes(branchNode, path, options) {
    if (config.excludeNonRelevant) {
      ...
      const isRepeatChild = repeatPath && path.startsWith(`${repeatPath}/`);
      const hasRepeatData = isRepeatChild && repeatIndex != null;

      const closestRepeat = branchNode.parentNode?.closest('.or-repeat');
      const checkRepeatIndex = repeatIndex == null && closestRepeat != null;
      ...
      for (const node of modelNodes) {
        ...
        const currentValue = isLeafNode
          ? node.textContent ||
            (relevanceState.get(node)?.currentValue ?? node.textContent)
          : null;
        ...
        if (setRelevant) {
          if (isLeafNode && (isReferencedNode || !isSelfNonRelevant)) {
            node.textContent = currentValue;
          }

          relevanceState.set(node, {
            isParentNonRelevant: isReferencedNode ? isParentNonRelevant : false,
            isSelfNonRelevant: isReferencedNode ? false : isSelfNonRelevant,
            currentValue,
            nonRelevantValue: isReferencedNode ? null : currentValue,
          });
        } else {
          if (isLeafNode) {
            node.textContent = '';
          ...
          relevanceState.set(node, {
            isParentNonRelevant: isReferencedNode ? isParentNonRelevant : true,
            isSelfNonRelevant: isReferencedNode ? true : isSelfNonRelevant,
            currentValue,

  ...
  implements OnDestroy
{
  ...
  public handleDropdownOpen() {
    if (!this.params().optionsBadge) return;
  ...
  private decorateDropdownOptionsWithBadges() {
    const optionsBadge = this.params().optionsBadge;
    const optionsValue = this.params().optionsValue;
    ...
    const badgeByLabel = this.answerOptions().reduce<Record<string, string>>((acc, option) => {
      ...
      const optionBadge = option?.[optionsBadge];
      if (optionLabel && optionBadge) {
        acc[String(optionLabel).trim()] = String(optionBadge).trim().toUpperCase();
      }
      return acc;
    }, {});

...
const parameters = () =>
  defineParameters({
    disabled: new Parameter("disabled", false, "script"),
    numberInput: new Parameter("number_input", false, "script"), // todo: this looks like something that we could remove / replace
    maxLength: new Parameter("max_length", -1, "script"),
...
export class TextBoxComponent extends RowBaseComponent<ReturnType<typeof parameters>> {
  public async handleChange(value: any) {
    await this.setExpression(value);
  }
}

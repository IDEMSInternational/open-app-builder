...
export async function createGoogleSheet(options: ICreateSheetOptions): Promise<ISheetResult> {
  ...
  const { auth } = await authorizeGoogleSheets({ credentialsPath, authTokenPath });
...
export async function readGoogleSheet(
  ...
  const sheetsToRead =
    ranges.length > 0 ? ranges : sheets.map((s) => s.properties?.title || "");
  ...
  for (const sheetName of sheetsToRead) {
    ...
    result[sheetName] = (valuesResult.data.values as string[][]) || [];
  }

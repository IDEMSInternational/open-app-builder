...
const set_splash_image = async (assetsFolderPath: string, subPath: string) => {
  ...
  if (!availableAssets.includes('splash.png')) {
    return logError({
      ...
      msg2: `An splash.png file is required to generate a splash screen.`,
    })
  }
...
const set_launcher_icon = async (assetsFolderPath: string, subPath: string) => {
  ...
  if (availableAssets.includes('icon.png')) {
    iconSources.push(
      path.join(androidAssetsPath, 'icon.png')
    )
  } else {
    return logError({
  ...
  if (availableAssets.includes('icon-background.png') && availableAssets.includes('icon-foreground.png')) {

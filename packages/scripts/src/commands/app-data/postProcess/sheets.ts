...
class SheetsPostProcessor {
  ...
  public run() {
    ...
    this.copyCoreAssetPack(app_data.output_path, appSheetsFolder);
  ...
  // The core asset_pack file is generated by the assets post-processor. Here it is copied
  // back into the appropriate sheets folder, which is emptied at the start of this.run()
  private copyCoreAssetPack(appDataFolder: string, appSheetsFolder: string) {
    const flow_type = "asset_pack";
    const flow_name = "core_assets";
    const sourcePath = path.resolve(appDataFolder, `assets/${flow_name}.json`);
    const targetPath = path.resolve(appSheetsFolder, flow_type, `${flow_name}.json`);

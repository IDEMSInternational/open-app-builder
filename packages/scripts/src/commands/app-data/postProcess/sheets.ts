...
class SheetsPostProcessor {
  ...
  public run() {
    ...
    this.hackGenerateCoreAssetContentsDataList(app_data.output_path, appSheetsFolder);
  ...
  // HACK: the assets contents file is generated by the assets post-processor.
  // Here it is converted to data_list format and copied over to the sheets folder
  private hackGenerateCoreAssetContentsDataList(appDataFolder: string, appSheetsFolder: string) {
    const filePath = path.resolve(appDataFolder, "assets/contents.json");
    const contents = fs.readJsonSync(filePath);
    const contentsArray = [];
    for (const [key, value] of Object.entries(contents) as any) {
      contentsArray.push({ id: key, ...value });
    }
    const flow_type = "asset_pack";
    const flow_name = "core_assets";
    const assetsContentsDataList: FlowTypes.AssetPack = {
      flow_type,
      flow_name,
      rows: contentsArray,
    };
    const assetContentsPath = path.resolve(appSheetsFolder, flow_type, `${flow_name}.json`);
    fs.ensureDirSync(path.dirname(assetContentsPath));
    fs.writeFileSync(assetContentsPath, JSON.stringify(assetsContentsDataList, null, 2));
  }

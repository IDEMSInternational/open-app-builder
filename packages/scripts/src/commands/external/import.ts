...
program
  .description("Import files from an external location to a deployment")
  .argument("<sourcePath>", "Path to the external source directory")
  .action(async (sourcePath: string) => {
    await importExternalDeployment(sourcePath);
  });

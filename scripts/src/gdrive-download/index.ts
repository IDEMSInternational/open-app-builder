...
async function main() {
  try {
    ...
    if (fs.existsSync(gdriveAssets)) {
      ncp(gdriveAssets, path.join(__dirname, "../../src/assets"), (err) => {
...
async function getPLHFolderId(drive: drive_v3.Drive): Promise<string> {
  const res = await drive.files.list({
    q:
      "mimeType='application/vnd.google-apps.folder' and name contains 'PLH' and name contains 'Excel'",
    pageSize: 1,
...
async function downloadSingleFile(
  ...
  if (exportMimeTypeMapping[file.mimeType]) {
    try {
      const mimeType = exportMimeTypeMapping[file.mimeType];
...
async function downloadFilesRecursively(
  ...
  const files = res.data.files.filter((file) => file.mimeType !== GOOGLE_FOLDER_MIMETYPE);
  await Promise.all(
    files.map((file) => {
  ...
  const subFolderRequests = subFolders.map((file) =>
    downloadFilesRecursively(drive, file.id, path.join(outputPath, file.name))
  );

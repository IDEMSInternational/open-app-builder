...
import { ConversationExcelRow } from './conversation.row.model';
...
export function main() {
    ...
    try {
        ...
        let exportObject = processWorkbook(workbook, outputFolderPath);
...
export function processWorkbook(workbook: xlsx.WorkBook, outputFolderPath: string) {
    let contentList: {
        Sheet_Name: string,
...
export function getFromRows(row: ConversationExcelRow, rows: ConversationExcelRow[]): ConversationExcelRow[] {
    return getFromRowIndices(row, rows).map((i) => rows[i]);
}

export function getFromRowIndices(row: ConversationExcelRow, rows: ConversationExcelRow[]): number[] {
    if (row.From) {
...
export function getFromNodes(row: ConversationExcelRow, rows: ConversationExcelRow[]) {
    return getFromRowIndices(row, rows)
        .map((i) => rows[i].rapidProNode)
        .filter((node) => node !== undefined);
}

export function getRoutersFromRow(currentRow: ConversationExcelRow, rows: ConversationExcelRow[], nodesById: { [nodeId: string]: RapidProFlowExport.Node }) {
...
export function attachToUnattachedCategories(routerNode: RapidProFlowExport.Node, newExit: RapidProFlowExport.Exit) {
...
export function processRouterRow(
...
export function getRowChoices(row: ConversationExcelRow) {
